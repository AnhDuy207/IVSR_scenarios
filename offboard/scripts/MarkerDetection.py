...
class MarkerDetector:
    ...
    def __init__(self):
        ...
        mavros.set_namespace('mavros')  # initialize mavros module with default namespace / Sets namespace of mavros node
        ...
        self.frame = np.zeros((540, 960, 3), np.uint8) # khoi tao ma tran x,y,z = 540, 960, 3 tai moi pixel co gia tri bieu dien tu 0 den 255
        self.pos = [0.0] * 4 #khoi tao ma tran 1x4 voi cac gia tri la 0.0
    ...
    def Aruco_marker_detector(self):
        ...
        while not rospy.is_shutdown():
            ...
            #rejectedCandidates => rejectedImgPoints
            ...
            if np.all(markerIds != None):
                ...
                for i in range(0, markerIds.size):
                    ...
                    if markerIds[i][0] == self.ids_target[0]:
                        ...
                        # decide move or descend
                        check_move = self.check_angle(alpha)
                        self.check_move_position.publish(check_move)

                        # marker in the body (UAV frane)
                        marker_pos = PS()
                        tvec2 = np.matmul(self.imu_cam, tvec1)   # The numpy.matmul() function returns the matrix product of two arrays.
                        ...
                        frame_out = cv.aruco.drawAxis(img, self.K, self.distCoeffs, rvecs, tvecs, axisLength) # to get world coordinate system axis for object points
                        # self.aruco_marker_pos_pub.publish(marker_pos)
